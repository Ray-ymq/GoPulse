...
Phase 8-01 keeps Monitor's publishing contract unchanged and adds the downstream Marshaller/VictoriaMetrics closure. `scripts/verify-marshaller.sh` is the real Redis → Exporter → Monitor → Router → Kafka → Marshaller → VictoriaMetrics acceptance; its only fixture-produced record is a representative permanent invalid Kafka message used to prove safe continuation.
