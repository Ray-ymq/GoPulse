...
func OfficialCatalog() []CatalogEntry {
	...
	for i, source := range sources {
		entries[i] = CatalogEntry{ID: source + "-exporter", Source: source, TargetID: source + "-exporter-local", Entrypoint: "bin/gopulse-" + source + "-exporter", Port: 9121 + i, HostPort: ports[i], ContainerPort: ports[i], Available: i < 3}
		if source == "kafka" {
