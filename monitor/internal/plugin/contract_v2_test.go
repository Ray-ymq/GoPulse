...
func TestOfficialCatalogIdentityAndAvailability(t *testing.T) {
	...
	for i, entry := range catalog {
		if entry.ID != sources[i]+"-exporter" || entry.Source != sources[i] || entry.TargetID != entry.ID+"-local" || entry.Entrypoint != "bin/gopulse-"+entry.ID || entry.Port != 9121+i || entry.Available != (i == 0) {
