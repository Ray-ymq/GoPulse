...
Phase 8-01 adds Marshaller as the formal downstream consumer of the unchanged record contract. Router still does not parse metrics payloads or commit consumer offsets; Marshaller independently validates the original bytes, writes accepted metrics to VictoriaMetrics, and commits through `gopulse-marshaller-metrics-v1`.
