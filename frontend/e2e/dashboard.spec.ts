...
test('real overview, catalog rule lifecycle, users and audit',async({page})=>{
 ...
 await expect(page.locator('[data-section="components"] tbody tr')).toHaveCount(6)
 ...
 await page.getByRole('button',{name:'rules',exact:true}).click()
 const row=page.locator('[data-rule-id]').first();const stoppedName=(await row.locator('h3').innerText()).split(' · ')[0]!;page.once('dialog',d=>d.accept());await row.getByRole('button',{name:'停用',exact:true}).click()
 await page.getByRole('button',{name:'history',exact:true}).click();await page.getByRole('button',{name:stoppedName,exact:true}).click();await expect(page.getByRole('complementary',{name:'告警详情'})).toContainText('rule_disabled')
