...
type Application interface {
	Bookmark(context.Context, uint64, uint64) error
	Unbookmark(context.Context, uint64, uint64) error
}
...
func NewHandler(application Application, loggers ...*slog.Logger) *Handler {
	...
	return &Handler{application: application, logger: logging.Module(logger, "bookmark")}
}

func (handler *Handler) Bookmark(c *gin.Context) {
	handler.apply(c, handler.application.Bookmark, "post bookmarked")
}

func (handler *Handler) Unbookmark(c *gin.Context) {
	handler.apply(c, handler.application.Unbookmark, "post unbookmarked")
}
...
func (handler *Handler) apply(c *gin.Context, operation func(context.Context, uint64, uint64) error, message string) {
	...
	logging.Module(logging.FromContext(c.Request.Context(), handler.logger), "bookmark").Info(message, slog.Uint64("user_id", userID), slog.Uint64("post_id", postID))
