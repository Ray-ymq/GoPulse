...
func TestIntegrationAuthenticationHTTPFlowAndApplicationRestart(t *testing.T) {
	...
	if _, err := database.ExecContext(context.Background(), `UPDATE users SET role='super_admin' WHERE username=?`, username); err != nil {
		_ = database.Close()
		t.Fatalf("promote current user: %v", err)
	}
	promotedCurrent := performJSONRequest(router, stdhttp.MethodGet, "/api/v1/users/me", "", cookies[0])
	if promotedCurrent.Code != stdhttp.StatusOK || !strings.Contains(promotedCurrent.Body.String(), `"role":"super_admin"`) {
		_ = database.Close()
	...
	if login.Code != stdhttp.StatusOK || len(login.Result().Cookies()) != 1 || login.Result().Cookies()[0].Value == "" || !strings.Contains(login.Body.String(), `"role":"super_admin"`) {
