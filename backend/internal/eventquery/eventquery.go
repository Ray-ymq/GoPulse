...
func validFilters(filters Filters) bool {
	...
	if filters.PluginID != "" && (filters.PluginID != "redis-exporter" && filters.PluginID != "mysql-exporter" && filters.PluginID != "rabbitmq-exporter") {
		return false
	}
...
func validDocument(name, source, severity, message string, m Metadata) bool {
	...
	if source != "monitor" || messages[name] != message || severities[name] != severity || (m.PluginID != "redis-exporter" && m.PluginID != "mysql-exporter" && m.PluginID != "rabbitmq-exporter") {
		return false
	}
