...
func EncodeCursor(cursor Cursor) (string, error) {
	...
	payload, err := json.Marshal(cursorPayload{
		Version: cursorVersion, CreatedAt: cursor.CreatedAt.UTC().Format(time.RFC3339Nano), ID: cursor.ID,
	})
	if err != nil {
		return "", err
	}
...
func DecodeCursor(token string) (Cursor, error) {
	...
	if payload.Version != cursorVersion || payload.ID == 0 || !strings.HasSuffix(payload.CreatedAt, "Z") {
