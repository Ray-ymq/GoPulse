...
- 本阶段把 Phase 13 交付的完整跨平台容器产品迁移到规划的 Kubernetes 集群，建立服务发现、必要持久化和基本调度能力。
...
- 部署 Frontend、Backend、Worker、自研可观测组件和 Phase 13 官方插件。
...
- Phase 13 跨平台产品化与插件扩展完成，且 Phase 12 容器运行契约仍有效。
...
- 完整 GoPulse 及六类官方单实例插件可脱离 Docker Compose 在 Kubernetes 中运行。
- 所有组件通过 Service 名称通信。
- 工作负载按预期分布到三个 Worker。
- 必要数据在 Pod 重建后仍可使用。
...
- 达到上述条件后停止扩展；复杂高可用和生产级有状态治理转入后续事项，集群内 Service 与工作负载作为 Phase 15 的输入。
