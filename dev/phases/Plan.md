...
- 默认安排 1～2 个实现批次，再固定保留 1 个 Review、集成验收与阶段收口批次，总批次数尽量控制在 2～3 个。
- 超过 3 个批次时，必须在总实施方案中记录具体的风险隔离、依赖关系或独立交付理由。
- 实现批次按可运行、可验证的端到端能力切分，不按数据库、Backend、Frontend、测试等技术层机械拆分。
- 测试、文档和实施记录随对应能力完成；收口批次只执行跨批集成、固定验收门槛和实际 Review 整改，不引入新的功能范围。
- 阶段验收通过且无 P0/P1 阻塞后立即停止；非阻塞优化和非当前 MVP 必需内容进入后续事项。
