...
> 当前状态（2026-09-13 更新）：Phase-15-01 至 Phase-15-06 已合入主线。第六批主线提交为 `5cf850b1aa82e2dc4a298c60e5614db53cdc9f49`（`origin/main`，版本 `1.12.6`）；第七批在用户指定权威分支 `develop/1.12.7` 完成 Review 整改与定向验收，完成产品版本为 `1.12.7`，尚不等于已合入主线。原六批 Compose 验收记录保持有效且不改写历史，本次不宣称重新完成完整 Compose 验收。

本文档原于 2026-09-12 基于主远程 `upstream/main` 提交 `53615805aaa73190121be35e2e7b54b4af4d3b3a`、Phase 14 已完成产品版本 `1.11.7` 编写。Phase 15 使用 `1.12.x` 版本线，现拆分为 7 个执行批次。本文档是批次顺序、目标版本和开发分支的唯一权威来源。第七批延续本次 Review 的指定分支；后续独立任务仍遵守从最新远程主线创建版本分支的规则。
...
Phase 15 的 patch `0` 是阶段分配基线，不表示当前产品已是 `1.12.0`，也不创建空提交。7 个执行批次按下表顺序实施：
...
| Phase-15-07 | `1.12.7` | `develop/1.12.7` | Review 三源过滤、插件健康优先级、管理 DTO 校验整改及直接回归 |
...
## Review 整改补充验收（Phase-15-07）

前三项整改必须共同满足：三源列表与签名分页进入真实 repository 并返回对应来源；插件已确认失败不被新鲜成功样本覆盖且管理大屏接受固定原因码；告警、用户、审计页面只接收字段精确的 DTO，畸形响应不进入操作或展示状态。固定验证与停止条件见 `Phase-15-07-Review整改.md`，实际结果见对应实施日志。此补充不替代前六批已记录的端到端与 Milestone 4 条件，也不增加 Kubernetes 或跨平台前置门禁。
