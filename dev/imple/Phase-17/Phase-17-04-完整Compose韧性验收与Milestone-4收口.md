# Phase-17-04：完整 Compose 韧性验收与 Milestone 4 收口实施方案
...
本批不首次实现产品功能。它冻结同一个 `1.14.4` Linux `amd64` 候选，从独立版本化 Bundle 运行 Phase 17 最终产品韧性矩阵，聚合可机器校验的脱敏 evidence，确认完整 Compose 产品无需 Kubernetes 即可成立，并完成 Milestone 4 收口。

```text
one 1.14.4 manifest + bundle + runtime contract
                         │
                         ▼
...
- Phase-17-01/02/03 已按顺序合入最新 `upstream/main`，同名实施记录、版本、固定门禁和结构化 receipt 可复核。
- 从该主线运行 `scripts/start-development-batch.sh Phase-17-04 --remote upstream` 创建 `develop/1.14.4`。
- `1.14.4` release manifest、Bundle、runtime contract、9 个产品镜像、lifecycle 镜像、6 个 current plugin 和 6 个第三方 image 绑定同一 Git revision/digest 集合。
...
缺少真实 server、同候选 digest、前序数据或隔离目录时，阻断最终收口，但不回滚前三批已通过且输入未变的结果。

## 3. 实施范围

### 3.1 候选冻结与预检

- 构建并发布 `1.14.4` Linux `amd64` 候选后冻结 release manifest、Bundle checksum、runtime contract digest、镜像/plugin digest 和 Git revision。
...
- 聚合器只接受同一 `1.14.4` manifest、Bundle、runtime contract 和 Git revision 的 receipts；Phase 16/前三批 evidence 只可作为可追溯前置，不可直接填充最终候选运行结果。
...
- 使用固定 `1.13.6` manifest 在隔离 project 通过正式入口生成当前数据并备份；恢复后由 `1.14.4` migration job 推进，验证 identity、role、social、search、plugin、metrics/logs/events、alert 和 audit 事实以及新增写入。
...
- 若 Phase-17-03 未新增 Schema，明确记录 target 与 `1.13.6` 相同，但仍保留 CLI、状态、直接前序数据和拒绝路径证据；不创建占位 migration。
...
- 因本阶段直接修改配置、Schema 检查、进程退出和消费者状态，使用 `1.14.4` 当前候选运行一次 Phase 16 format v1 backup/inspect/空 project restore 与恢复后写入回归。
...
- 确认 Phase-17-01/02/03 实施记录齐全、版本/分支顺序正确、根与受管版本为 `1.14.4`。
...
1. 确认真实 host/server、独立目录/project 和前三批证据，冻结 `1.14.4` 候选。
...
- `dev/logs/Phase-17/Phase-17-04-完整Compose韧性验收与Milestone-4收口.md`
- `VERSION`、`.env.example` 和双 Frontend 版本元数据

本批不得以“阶段收口”为由重写前三批已通过的实现。
...
2. 全部场景绑定同一 `1.14.4` Git revision、release manifest、Bundle checksum、runtime contract 和 image/plugin digest。
...
4. Phase 17 四份同名实施记录齐全且只记录真实结果；根完成版本为 `1.14.4`，四批按顺序合入主线，无阻断问题。

以上全部通过才完成本批、Phase 17 和 Milestone 4；任一 Kubernetes 结果都不能替代这些条件。

## 8. 固定验证命令与回归范围

最终候选固定入口如下；新增脚本由前三批/本批按计划落地，等价调整必须同步所有计划和实施记录：
...
python3 scripts/ci/validate_branch.py --branch develop/1.14.4 --base-ref upstream/main
git diff --check
```
...
完成前创建 `dev/logs/Phase-17/Phase-17-04-完整Compose韧性验收与Milestone-4收口.md`，至少记录：
...
交给 Phase 18 的固定输入是 `1.14.4` Linux `amd64` 同一完整产品、运行时合同、三类 Probe、关停预算、Schema/Migration、consumer/alert 可靠性和最终 Compose evidence。Phase 18 只改变部署位置与编排方式，不得补做产品功能；达到上述条件后立即停止。
