...
建立跨平台、非破坏性的运行状态验收脚本，并单独执行 Phase 0 的完整冷启动、故障、恢复和停止验证，补齐开发入口文档与阶段记录，形成可交付给 Phase 1 的工程骨架。
...
`verify` 仅检查已运行环境，不启动或停止应用、容器和业务依赖，不制造故障，也不删除数据卷。完整冷启动和故障矩阵按 3.2、3.3 作为独立验收流程执行并在实施记录中留存证据。
...
5. Frontend 页面可访问，并通过浏览器自动化或明确记录的人工浏览器验收确认展示四项正常状态；不得仅以首页 HTML 返回 200 代替渲染结果验证。
...
前四批的实施记录应已由对应批次各自创建；本批只创建或更新同名的 Phase 0-05 记录，不使用一份总记录替代 01～04 的对应记录。
...
4. 实现 Frontend HTTP 可访问性检查；将“页面实际展示的状态正确”保留给组件测试和浏览器验收，不由 HTTP 文本断言伪装覆盖。
5. 为每项失败输出明确名称、实际结果和建议诊断命令，同时避免泄露敏感配置。
6. 执行 Backend 全部测试和静态检查。
7. 执行 Frontend 测试、类型检查和生产构建。
8. 从冷状态执行完整主流程，在全部正常的运行状态下调用非破坏性 `verify`。
9. 在 `verify` 之外单独执行单项、多项故障、恢复、重复启动、幂等停止、端口冲突和陈旧运行记录场景，每个破坏性场景完成后恢复到已知状态。
