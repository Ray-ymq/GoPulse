...
- `unreachable`：对应接口无法访问。
- `invalid`：接口可访问，但返回非预期状态码或不符合契约的响应。
- `unknown`：Backend 存活，但 readiness 请求超时、返回非预期状态码或响应结构无效，因而无法确定基础设施状态。
...
1. 在统一刷新函数中同时发起两个请求，每个请求使用 3 秒客户端超时。
2. Backend 状态只由 `/health` 决定：仅当返回 200 且 JSON 契约正确时标记为 `up`；网络失败或超时标记为 `unreachable`，其他响应标记为 `invalid`。
3. `/ready` 的 200 和 503 都应解析响应体；503 是合法业务状态，不等同于网络错误。
4. 只有状态码为 200 或 503 且完整响应契约通过校验时，才根据 `checks.mysql`、`checks.redis`、`checks.rabbitmq` 映射三项状态。
5. `/health` 成功但 `/ready` 网络失败、超时或契约无效时，Backend 仍显示正常，三项依赖改为 `unknown`，不保留误导性旧状态。
6. `/health` 不可达时，Backend 显示 `unreachable`；即使 `/ready` 存在独立结果，也不将 Backend 标记为正常，三项依赖应显示为无法可靠判定。
7. 刷新期间禁用按钮，并使用请求序号或取消机制保证只有最新一轮请求能更新页面。
8. 错误信息使用面向开发者的简短描述，不显示敏感连接信息。
...
11. 在对应的 `dev/logs/Phase-00/Phase-00-03-Frontend连通性页面.md` 中记录实际完成内容、变更文件、验证命令与结果、偏差和已知限制。
...
- `/health` 正常但 `/ready` 超时、返回非 200/503 或契约错误时，Backend 仍显示正常，三项依赖显示未知。
