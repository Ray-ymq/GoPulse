...
| Phase-00-04 | `0.1.4` | `develop/0.1.4` | 待实施 |
| Phase-00-05 | `0.1.5` | `develop/0.1.5` | 待实施 |

若实施前调整批次数量或顺序，先更新本表，再创建尚未开始的开发分支；已经推送的分支不得静默改名或重新编号。

## 当前实施基线

- 根 `VERSION` 当前为 `0.1.3`，Phase-00-01 至 Phase-00-03 已合并到主线，对应实施记录已存在。
- 已交付 Compose 基础设施、Backend `/health` 与 `/ready`、Frontend 连通性页面及其自动化测试。
- 剩余工作只包含 Phase-00-04 的跨平台开发脚本和 Phase-00-05 的集成验收与阶段收口，不重做前三批已验收能力。
- Phase-00-04 与 Phase-00-05 实施时，分别从配置的主远程最新 `main` 创建表中指定的分支；前一批合并主线后才开始后一批。
- 每批完成时将 `VERSION` 更新为本表分配的目标版本，并与本批实施记录一起提交。
