...
- 02-06 关闭 2026-09-02 Phase 2 实现 Review 的 P2/P3 项，不扩大到新业务能力或 Phase 3 搜索实现。
...
### 10.6 [Phase-02-06：实现 Review 整改](Phase-02-06-实现Review整改.md)

- 为已发布 Outbox 行启用有界保留清理，并增加配置、文档和最低层运行时证明。
- 约束整批 publish 时间预算小于租约，避免慢 Broker 系统性耗尽 batch 租约。
- 让 Worker shutdown 取消并回收在途 handler，消除返回后的 goroutine/redelivery 重叠。
- 收口 Phase 2 权威状态，并阻止自动 PR 流程为无内容分支创建误导性提交。
...
| Phase-02-06 | Outbox Dispatcher/config、Worker runtime 和自动 PR guard 定向测试 | Backend 默认测试/vet/race、治理测试、版本/分支校验、Frontend 既有门禁和 Bash/Compose 固定检查 | 不重跑完整浏览器/故障矩阵，除非定向或固定门禁暴露跨组件回归；不重写既有 Git 历史 |
...
- 六份实施记录存在且与实际提交、命令、结果和限制一致；Phase 2 里程碑版本为 `0.3.5`，Review 整改完成版本为 `0.3.6`，根 `VERSION` 与 Frontend npm 元数据一致。
...
记录必须包含实际完成内容、文件、命令和结果、偏差、已知限制与跟进项。总方案本身不提前创建空实施记录，也不把规划验收表述为已经通过。
