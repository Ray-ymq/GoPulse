...
Phase 1 整体视为一个独立开发任务，所有实施批次共享同一个目标版本和开发分支，不按批次重复升版。

- 开始实施前从最新 `origin/main` 创建开发分支。
- Phase 1 是向后兼容的业务功能增量，默认属于 minor 版本影响。
- 从开始实施时的当前版本计算唯一目标版本，分支名使用 `develop/x.x.x`。
- 如实施范围改变，按仓库规则在首次 push 前重新评估唯一目标版本。
- 阶段收口时才将根 `VERSION` 更新为本任务目标版本。
...
- 六个批次的实施记录与实际工作一致。
- 根 `VERSION` 已更新为本任务唯一目标版本，且仅提交 Phase 1 相关文件。
