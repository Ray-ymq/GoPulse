...
- 开始前获取最新远端状态，从 `origin/main` 创建 Phase 1 唯一的 `develop/x.x.x` 分支。
- 已根据根 `VERSION` 或有效 SemVer 标签确定 Phase 1 的唯一 minor 目标版本；如两者都不存在，必须先请用户确定初始版本。
...
2. 确定 Phase 1 唯一目标版本，从最新 `origin/main` 创建对应开发分支。
