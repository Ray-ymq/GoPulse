...
在 Phase-08-01 已交付真实 Kafka → Marshaller → VictoriaMetrics 查询闭环的基础上，完成正式 Consumer 在 rebalance、commit 失败、Kafka/VictoriaMetrics 故障和进程重启条件下的可靠性能力，并把 Marshaller/VictoriaMetrics 纳入安全、可恢复的日常 Bash 生命周期，形成：
...
本批是第二个实现批次，不是纯测试或阶段收口。必须交付真实的 ownership fencing、故障恢复和运维生命周期增量；Phase-08-03 只在最终能力上完成跨组件、业务隔离和里程碑验收。
...
- Marshaller 已有正式 group `gopulse-marshaller-metrics-v1`、手动 offset、严格 Envelope/metrics validator、确定性 transformer、VictoriaMetrics client 和最小日常生命周期。
...
### 3.1 Partition ownership 与 rebalance fencing

- 每个在途 record 绑定当前 assignment generation 的 ownership lease；Writer、退避和 Committer 都接收该 lease 的 context。
...
- 通过注入 Consumer、Committer、Writer 和 ownership lease，确定性覆盖 HTTP acceptance/commit 失败、revoke/lost、延迟 `204`、提交竞态、退避取消和 shutdown；测试 seam 不得形成生产 HTTP 或普通运行配置。
...
- Marshaller CI 增加 ownership/commit 确定性测试和适合 CI 的真实 Kafka/VictoriaMetrics 故障恢复场景。
...
2. 为 Consumer、Committer、Writer 和 ownership 建立最小接口与 generation lease，不增加生产故障开关。
3. 实现 revoke/lost 取消、HTTP acceptance 后 ownership 再确认、同步 commit 和失败后安全重取。
4. 用注入测试确定性覆盖 commit 失败、延迟响应、ownership 竞态、退避取消和 shutdown。
...
- **本批退化为收口批次**：必须提交 ownership fencing、恢复或生命周期的真实实现增量；纯验收留给 Phase-08-03。
