...
| 批次 | 目标版本 | 分支 | 交付主题 | 状态 |
| --- | --- | --- | --- | --- |
| Phase-18-01 | `2.0.1` | `develop/2.0.1` | 高并发数据配方、负载工具与容量基线 | 待实施 |
...
完成条件是数据可重复、负载器不是瓶颈、报告绑定候选和宿主，并记录第一瓶颈；本批不为追数字做产品优化。

### Phase-18-02：多副本与所有权

交付 scale overlay、唯一 edge 负载分发、Kafka 4 partitions、多副本状态指标和替换验收。
完成条件是表 6 拓扑成立，每个组件有水平扩容或单所有者结论，相对扩容达到 1.3 倍，替换无丢失或重复副作用。

### Phase-18-03：存储隔离与背压

交付双 ES、配置/凭据分离、单 ES 升级、backup format v2 与全链路容量上限。
完成条件是交叉故障不扩散，迁移/恢复不丢失历史，超载无 OOM/无界增长/伪成功，回落后 10 分钟内收敛。
...
1. 固定宿主、数据、混合负载和同一候选的三轮结果满足第 5 节。
2. 五类可扩展组件通过多副本、替换和 1.3 倍相对扩容门禁，Monitor 单所有者安全拒绝第二实例。
3. Outbox/Rabbit/Kafka/alert 无越权提交、重复持久副作用或静默丢失已接受数据。
