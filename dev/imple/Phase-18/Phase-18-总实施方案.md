...
> 收口；`capacity-failure.json` 是后续批次的正式输入，不是通过 evidence。严格
> 重复性、完整收敛和阶段 SLO 统一保留为最终候选门禁。
>
> 规划修订（2026-09-25）：原 Phase-18-02 正式矩阵暴露 MySQL/Kafka parser、启动与稳态计时、
> replacement 工作保持和失败现场缺失等验收基础设施风险，不能继续用重复候选筛选结果。
> 新增 Phase-18-02“扩容验收基础设施可信度闭环”；原 Phase-18-02 至 18-05 依次顺延为
> Phase-18-03 至 18-06，目标版本和未创建开发分支同步顺延。原失败候选只作为 fixture 与诊断输入，
> 不追认为新批次 evidence。
...
| 存储 | SSD/NVMe 后端，验收前可用空间不少于 80 GiB |
...
| Phase-18-02 | `2.0.2` | `develop/2.0.2` | 扩容验收基础设施可信度闭环 | 待实施（方案已落地，本轮不执行） |
| Phase-18-03 | `2.0.3` | `develop/2.0.3` | 多副本扩容、负载分发与任务所有权 | 待实施（原 18-02 顺延） |
| Phase-18-04 | `2.0.4` | `develop/2.0.4` | 双 Elasticsearch 隔离与端到端背压 | 待实施（原 18-03 顺延） |
| Phase-18-05 | `2.0.5` | `develop/2.0.5` | 独立健康通道与合同单一来源 | 待实施（原 18-04 顺延） |
| Phase-18-06 | `2.0.6` | `develop/2.0.6` | 高并发、故障恢复与 Milestone 5 收口 | 待实施（原 18-05 顺延） |

每批必须从前一批合入后的最新 `upstream/main` 开工，使用
`scripts/start-development-batch.sh Phase-18-0X --remote upstream`。本表是批次、版本和分支唯一权威映射。

规划时远端不存在 `develop/2.0.2`；同名本地分支保存原多副本失败探索，不是新 Phase-18-02 的
合法开工基线。本轮只更新方案，不移动或删除该分支。实施新 Phase-18-02 前须在用户知情下将其
原样归档为非开发分支，再从当时最新 `upstream/main` 创建新的 `develop/2.0.2`。原候选、receipt、
执行次数和 `VERSION=2.0.2` 工作树状态不得跨批复用。Phase-18-03 只能在新 Phase-18-02 合入后
从主线创建 `develop/2.0.3`，原实现代码须逐项审计后重新移植。
...
Phase-18-02 先限定扩容测量资格：结构化 parser 正确、启动与稳态分离、固定 workload 受控释放、
真实 consumer ownership 可观察、单/多副本有效测量窗口均不少于 30 秒且至少 10 个样本、失败前
原子落盘完整现场，并能用资源和依赖样本区分 component/shared dependency/load generator。
资格 evidence 不代表产品扩容通过，也不生成 `scaling.json`。

相同依赖、初始数据和测量方法下，固定 8 vCPU 宿主上的 Backend 三副本必须完成固定
150/300 RPS 替换和持续服务门禁；由于该拓扑未对 Backend 副本实施 CPU 隔离，其单副本/三副本
饱和吞吐比值作为容量特征记录，不作为阻断门槛，也不能表述为水平容量增益。Worker、Indexer、
Router、Marshaller 的多副本可持续吞吐或积压排空速率默认至少为单副本的 1.3 倍。

Phase-18-03 的 Backend 扩容对比使用相同 1,024 用户池、相同混合请求比例、128 个并发执行者
轮流使用全部用户的无目标 RPS 测量：15 秒预热后连续测量 90 秒；每个执行者完成请求后立即发起
下一次请求，以稳定窗口成功完成数除以实测窗口时长计算吞吐。固定 150/300 RPS 窗口只用于容量
场景和 Backend 替换。其余组件必须使用 Phase-18-02 已取得资格的固定积压释放、成员观测和计时；
Router 另记录直接 Kafka ceiling 与并发阶梯。

正式扩容矩阵先完成五类组件的替换和所有权场景，再执行容量对比。各场景独立记录结果；单项失败不
阻止无依赖场景继续执行，但总体仍失败且不得生成通过 evidence。每个冻结候选只运行一次正式矩阵，
新候选必须对应已记录的产品、配置、资源合同或验收脚本变化，不重复未变化候选以筛选通过结果。
若 Phase-18-02 证明共享依赖 ceiling 使默认 1.3 门槛无法衡量组件扩容，必须在 Phase-18-03 开工前
先于 `update` 修订本方案并获得用户确认；不得在正式矩阵看到结果后降标。

同一候选连续三轮稳态的 RPS 偏差不得超过 ±5%，P95/P99 偏差不得超过 ±10%。

Phase-18-01 的单副本失败基线、Phase-18-02 的资格诊断和 Phase-18-03 的扩容对比只负责各自边界。
上述重复性、端到端收敛和完整 SLO 不以前序批次放宽或挑选成功轮次的方式收口，最终仍须由
Phase-18-06 在同一 `2.0.6` 候选上重新验证。扩容对比必须使用 Phase-18-03 内同候选、
...
- `1.14.5` 和 `2.0.3` 单 ES 升级时，保留原卷作为业务搜索输入，将 Logs/Events 模板、alias 和文档复制到观测 ES；
...
### Phase-18-02：扩容验收基础设施可信度

交付结构化 MySQL/Kafka/RabbitMQ 观测、启动与稳态分离计时、受控 workload release、确定性
replacement 工作保持、失败前原始附件和非 Backend 资源/依赖取样。完成条件是
`qualification.json` 与 `bottleneck-diagnostic.json` 绑定同一 `2.0.2` 候选，parser、计时、成员、
replacement 和清理资格门禁通过，且未生成或冒充 `scaling.json`。本批不修改产品扩容能力。

### Phase-18-03：多副本与所有权

交付 scale overlay、唯一 edge 负载分发、Kafka 4 partitions、多副本状态指标和替换验收。
完成条件是表 6 拓扑成立，每个组件有多副本运行或单所有者结论，Backend 完成固定负载替换、
持续服务和非阻断饱和容量记录，其余四类组件满足本批开工前已冻结的相对扩容门禁，
替换无丢失或重复副作用，并完成 Outbox 排空及所有权验证。旧 Phase-18-01 单副本轮次只作为问题
输入，原 Phase-18-02 失败探索只作为 fixture，均不参与扩容分母。重复性、完整收敛和端到端 SLO
不在本批收口。
...
完整 150/300 RPS SLO、重复性和跨系统最终收敛留给 Phase-18-06。

### Phase-18-05：独立健康与合同收敛
...
冻结一个 `2.0.6` manifest、Bundle、runtime contract、contract catalog digest 和全部 image/plugin digest，
运行稳态、突发、多副本替换、超载、双 ES 故障、追赶、直接前序迁移和备份恢复。
完成条件是全部 receipt 绑定同一候选，报告明确容量、第一瓶颈、单点和适用边界，`VERSION=2.0.6`。

## 11. 阶段验收与停止条件

1. Phase-18-01 的失败基线可复核；Phase-18-02 的 qualification 能区分产品与验收基础设施失败，
   parser、计时、成员、失败附件和瓶颈证据完整；Phase-18-03 不以任何旧失败轮次作为扩容分母。
2. 五类可扩展组件通过多副本和替换；Backend 在固定 150/300 RPS 替换窗口内持续服务并记录
   非阻断饱和吞吐特征，其余四类达到 Phase-18-03 开工前冻结的相对扩容门禁，Monitor 安全拒绝第二实例。
...
8. 六个批次的同名实施记录齐全，根版本为 `2.0.6`，没有阻断问题。

Phase-18-01 至 Phase-18-05 只交付各自边界内的可复核中间结果；最终重复性、收敛和完整 SLO
仍由 Phase-18-06 候选矩阵统一判定。
