# Phase-18-05：高并发故障恢复验收与 Milestone 5 收口实施方案
...
本批不首次设计产品能力。它冻结同一 `2.0.5` 候选，在固定 WSL2 Linux `amd64`、8 vCPU/12 GiB 环境中运行容量、多副本、背压、双 ES、诊断、合同、完整产品和恢复矩阵，发布脱敏容量报告并收口 Milestone 5。

若最终矩阵暴露阻断，只修直接缺陷，重建完整候选并使受影响证据失效；不删除场景、放宽 SLO 或用 Kubernetes 旁路通过。

## 2. 前置条件

- Phase-18-01/02/03/04 已按顺序合入最新 `upstream/main`，同名实施记录、版本和批次 evidence 可复核。
- 从该主线创建 `develop/2.0.5`，不复用任一前批分支。
- WSL2 可见配额精确为 8 vCPU、12 GiB RAM、8 GiB swap，工作区在 Linux 文件系统，可用 SSD 空间至少 100 GiB。
- 准备 clean、`1.14.5` migration、`2.0.4` direct-predecessor、fault、backup/restore 等独立 project 和随机归属 token。
...
- 对五类可扩展组件重跑单/多副本对比，要求至少 1.3 倍；逐实例替换并验证无越权提交或重复副作用。
...
- 从 `1.14.5` 正式候选创建单 ES 当前数据和 format v1 备份，升级/恢复到 `2.0.5` 双 ES，验证业务、搜索、观测、插件、角色、告警和审计事实。
- 对 `2.0.5` format v2 执行 backup/inspect/空 project restore，验证双 ES 与恢复后新写入。
...
只有全部场景通过、五份实施记录齐全、无阻断问题且 `VERSION=2.0.5` 时，才能完成 Phase 18 和 Milestone 5。

## 6. 批次与阶段完成条件

1. 同一 `2.0.5` 候选的宿主预检、三轮容量、多副本、替换、背压、双 ES、诊断/canary 和合同漂移全部通过。
2. `1.14.5 → 2.0.5`、format v1/v2 restore、恢复后新写入和完整产品/权限回归通过。
3. 最终 evidence 只包含同候选完整 receipt，Secret/路径/payload 扫描、强归属和无关资源对比通过。
4. 容量报告明确固定环境、第一瓶颈、可扩展组件、状态单点、超容量行为和适用边界。
5. Phase-18-01 至 18-05 同名实施记录齐全，根与受管版本为 `2.0.5`，无阻断问题。
...
创建 `dev/logs/Phase-18/Phase-18-05-高并发故障恢复验收与Milestone-5收口.md`，记录候选/宿主、所有实际命令与结果、
失败轮次与最小修复、receipt 复用依据、容量报告、偏差、限制和外部发布边界。完成后更新 `VERSION=2.0.5`，提交并停止。
