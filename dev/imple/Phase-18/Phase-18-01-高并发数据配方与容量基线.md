...
>
> **历史状态（2026-09-26）**：本批已结束，只保留为当时的执行合同。它交付了数据/负载工具和
> 失败诊断，但没有证明容量基线通过，不得再次按本文运行三轮。后续仅由 Phase-18-02
> 采纳已定位的 Outbox 修复并收口阶段结论。

## 1. 批次目标

本批建立确定性数据、负载、资源取样和 evidence 基础，并对当前单副本产品取得三轮未优化基线。
基线即使不满足阶段 SLO 也必须如实保留；2026-09-24 规划修订后，本批允许以
“失败基线已交付、重复性与恢复门禁未通过”收口。严格重复性、最终收敛和完整 SLO
不在本批追加重跑；2026-09-26 重划后由 Phase-18-02 采纳已定位修复并收口
`boundary_found` 结论。本批不为追逐 RPS 修改产品算法、Schema
或中间件参数。
...
以上命令和完成条件仅记录历史执行合同；新批次不得据此重跑或补造 Phase-18-01 通过证据。
