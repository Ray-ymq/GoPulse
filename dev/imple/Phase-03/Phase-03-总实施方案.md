...
- `1.0.0` 是三个 Phase 3 开发批次全部完成后的 Milestone 1 发布版本，不替代 `0.4.1`～`0.4.3` 的批次版本。

### 3.3 `1.0.0` 里程碑发布动作

Phase-03-03 合入主远程 `main`、远程门禁通过且第 14 节满足后，执行一次不计入 Phase 批次数量的 release-only 动作：
...
- Phase-03-03 先让分支校验器识别唯一 release-only 分配；不得把发布伪装成 `Phase-03-04`，也不得在 `update` 上修改 `VERSION`。
...
- [Phase-03-03：Review、集成验收与里程碑收口](Phase-03-03-Review集成验收与里程碑收口.md)：只做真实 finding 整改、跨批验收和发布准备。

两个纵向实现批次加一个固定收口批次符合阶段切分约束，不再按索引、Backend、Frontend 或测试机械增批。
...
| Phase-03-03 | 独立 Review finding、阶段矩阵一次 | Backend/Frontend/脚本门禁、远程 CI、版本/文档 | 不新增功能，不重复未受整改影响的定向测试 |
...
只有三批均从权威分支完成并合入主远程、封闭矩阵在 WSL2/Bash 真实通过、远程门禁成功、记录齐全且无 P0/P1，Phase 3 才可标记完成。随后立即执行 `1.0.0` release-only 动作并停止功能扩展；未执行检查不得写成通过。

向 Phase 4 交付已运行的 ES 基础设施与 readiness 模式、独立可重建的搜索索引契约、完整业务 MVP 和各进程的日志需求。Phase 4 只统一业务日志，不复用帖子搜索别名/Mapping，也不改写本阶段事实与消息语义。
