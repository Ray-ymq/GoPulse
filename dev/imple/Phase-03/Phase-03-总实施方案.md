...
本阶段按“先交付可重建的搜索读闭环，再接入可靠增量索引，最后统一 Review 与验收”的顺序执行：
...
- 根 `VERSION` 与 Frontend npm 元数据均为 `0.3.6`；Phase 2 及其实现 Review 整改已经完成。
...
- Phase-03-01 与 Phase-03-02 是两个可独立运行、可独立验证的纵向实现批次；Phase-03-03 固定用于 Review、跨批集成验收与阶段收口，不预装新功能。
...
- 发布分支从已验证的主远程 `main` 创建，只允许更新根/Frontend 版本元数据、里程碑状态、发布说明和直接需要的分支治理规则，不夹带产品功能或 Review 整改。
...
- Phase 0～3 必要业务回归、独立实现 Review、实施记录和 Milestone 1 发布准备。
...
Phase-03-02 可靠增量索引与运行闭环（0.4.2）
  ↓
Phase-03-03 Review、集成验收与里程碑收口（0.4.3）
  ↓
...
- [Phase-03-03：Review、集成验收与里程碑收口](Phase-03-03-Review集成验收与里程碑收口.md)：不新增范围，只整改真实阻断 finding、执行跨批矩阵并准备 `1.0.0` 发布。

两个纵向实现批次加一个固定 Review/收口批次符合阶段提纲的 2～3 批约束；每个实现批次都有用户可观察的完整闭环，不按 Elasticsearch、Backend、Frontend 或测试层机械拆分。
...
- 固定门禁通过且无 P0/P1 后立即更新实施记录、版本并提交，不追加机会性重构、压测、分词排列或搜索增强。
...
| Phase-03-03 | 独立 Review finding、阶段矩阵一次、真实浏览器闭环 | Backend/Frontend/脚本固定门禁、远程 CI、版本/文档 | 不新增功能，不重复未受整改影响的定向测试 |
...
以上是封闭矩阵；除非真实失败暴露新的 P0/P1 或共享基础设施风险，不追加 analyzer 黄金集、压力测试、节点故障排列或与本阶段无关的回归。
...
- 第 12.3 节固定矩阵和远程门禁通过，无 P0/P1；直接破坏阶段验收的 P2 已关闭，其他非阻塞问题已登记而未扩大实现。
...
只有 Phase-03-01 至 Phase-03-03 均从权威分支完成并合入主远程、第 12.3 节封闭矩阵在 WSL2/Bash 真实通过、远程门禁成功、实施记录齐全且无 P0/P1，Phase 3 才可标记完成。随后执行 `1.0.0` release-only 动作并停止功能扩展；未执行的检查、PR、合并、tag 或发布不得写成通过。
...
- Review、故障恢复和资源归属证据；Phase 4 不重新定义搜索事实来源或增量消息语义。
