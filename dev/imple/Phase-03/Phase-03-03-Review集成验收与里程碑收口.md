...
对 Phase 3 最终实现执行独立代码/架构 Review，最小整改真实阻断项，随后一次性完成索引删除重建、增量故障恢复、Frontend 用户闭环和 Phase 0～3 必要业务回归，形成 Phase 3 与 Milestone 1 的可审计收口证据。

本批是固定 Review、集成验收与阶段收口批次，不新增搜索功能。Phase 3 开发版本完成后为 `0.4.3`；`1.0.0` 只在本批合入且远程门禁通过后，按总方案执行独立 release-only 动作。
...
- MySQL/Elasticsearch 数据归属和 Search API hydration 是否存在反向事实依赖或 `_source` 越权使用。
- rebuild advisory lock、H1/H2、Bulk item error、alias 原子切换和精确删除是否可能丢数据或误删资源。
- posts + Outbox 原子性、Envelope 兼容、Publisher 路由、拓扑隔离、至少一次与 `_id` 幂等是否与文档一致。
- Search Indexer 的错误分类、manual ack、confirm-gated retry/dead、重连、alias 缺失和有界退出是否可靠。
- Phase 2 的 Outbox cleanup、整批租约预算、Business Worker shutdown 和通知幂等是否因本阶段改造发生回归。
- q/cursor/timeout/响应体上限/错误映射、认证、Frontend 直连边界和日志脱敏是否安全。
- Bash/CI 的端口、容器、network、volume、PID 和中断清理是否保持归属保护。
- Phase 0～2 能力与 Phase 4 交接是否被 Phase 3 破坏或错误耦合。

在 `dev/review/` 生成 Phase 3 实现 Review 报告，记录基线、范围、实际命令、证据、P0～P3 finding 和结论。不得把计划检查写成已执行，也不得用新增测试替代已发现的生产缺陷修复。

### 3.2 Finding 处理边界

- P0/P1 必须最小修复并重跑受影响门禁；无法关闭时 Phase 3 保持未完成。
- 直接使总方案验收不成立、导致数据丢失/越权/误删或破坏公共契约的 P2 作为收口阻断。
- 非阻断 P2/P3 在 Review 和实施记录中登记证据、风险与建议，移交后续，不扩大本批。
- 修复必须映射 Review finding 或失败验收；禁止机会性重构、依赖升级、视觉改版、搜索增强或后续 Phase 提前实现。
- 只为真实 finding 或失败增加最低有效测试，同一事实不在 unit、integration 和 E2E 三层重复证明。
...
- 核对总方案、三份拆分方案、三份实施记录、Review 报告、Git 历史、版本和分支权威分配。
- 本地门禁通过时只记录本地完成状态；只有实际查询到 PR 合入和远程门禁成功后，才能把总方案标记为 Phase 3 完成。
...
- 不因剩余时间做未映射 finding 的清理、重构或测试扩充。
...
预计文件是允许边界，不要求制造无意义修改。Review 没有生产代码 finding 时，本批可以只更新 Review、验收、治理、版本和收口文档。

## 6. 详细实施步骤

1. 冻结 Review 基线和 Phase 3 变更范围，核对前两批实施记录、远程门禁、接受限制与日常资源指纹。
2. 按第 3.1 节审阅直接相关生产代码、测试、脚本和公共契约，形成有文件/行号/命令证据的报告。
3. 对 P0/P1 与阻断 P2 选择最小修复，先运行最低层复现，再运行受影响命令；非阻断项登记后停止。
4. 最终 diff 稳定后执行 Backend、Frontend、脚本治理和 Compose 固定门禁各一次。
5. 从空隔离环境顺序执行总方案十项矩阵；每项故障恢复后先确认系统回到可验收状态，再进入下一项。
6. 复核搜索索引删除、alias swap、两个 Worker、RabbitMQ/Elasticsearch 重启和脚本中断不触碰日常资源。
7. 真实浏览器完成注册、发帖、评论、点赞、通知、历史搜索、新帖搜索、分页和详情跳转的必要旅程。
8. 查询实际远程 Pull Request/check 状态，只记录存在的 head commit、检查名称和结论。
9. 更新 README、总方案状态、本批实施记录、Review 结论和 `0.4.3` 版本元数据。
10. 更新并测试 release-only 分支校验，确认 `develop/1.0.0` 唯一映射、普通 Phase 分支规则不放宽、`update` 仍禁止版本变更。
11. 提交并创建/合并 `develop/0.4.3` Pull Request；远程门禁未通过时保持阶段未完成。
12. 合入与远程门禁通过后按总方案执行独立 `develop/1.0.0` release-only 动作，不再修改 Phase 3 产品代码。

## 7. 风险与控制

- **Review 演变成重做**：只处理有代码证据且映射阶段验收的 finding，非阻断建议登记后停止。
...
`scripts/verify-business.sh` 是一次性阶段级真实基础设施入口，必须包含总方案十项矩阵，因此不再另跑重复的全量 Playwright、Phase 2 故障脚本或额外 Elasticsearch 压力测试。Review 修复的定向复现可在固定门禁前执行，实施记录需区分定向证据和固定完成门禁。
...
- Review 覆盖既定范围，结论有真实代码、命令和运行证据，P0/P1 为零，阻断 P2 已关闭。
...
- 三份实施记录、总方案、Review、Git 历史和 `VERSION=0.4.3` 一致，Pull Request 已合入且远程门禁通过。
...
任一矩阵、远程门禁、实施记录或发布证据缺失时，不得写成完成。发布后停止 Phase 3 扩展，非阻塞搜索增强移交后续规划。
...
- 完整 Review、故障恢复和资源归属证据；Phase 4 不重新定义搜索事实来源或增量消息语义。
