...
- 初始化、启动、停止、状态、日志、只读验证、诊断、备份、恢复和升级由一个共享生命周期实现提供；平台可执行文件来自同一 Go module，不维护 Bash/PowerShell 两套业务逻辑。
...
- 三个原生生命周期 CLI 交付包：`linux/amd64`、`darwin/arm64`、`windows/amd64`，均由同一源码和命令合同构建。
...
| Windows `amd64` | 仓库 Phase 0 记录证明曾有原生 Windows/Docker Desktop 环境；本阶段按仓库平台规则重新安排真实 Windows 宿主 | NTFS 用户目录中的独立交付包；Windows PowerShell/Terminal 直接运行 `gopulse.exe`；Docker Desktop Linux containers | 当前规划会话不能替代 Windows 实机确认；必须记录 Windows build、Docker/Compose、Linux container mode、NTFS 路径、文件共享与可用资源 |
...
| --- | --- | --- | --- |
| Phase-16-01 | `1.13.1` | `develop/1.13.1` | 双架构产品镜像、六插件包、release manifest 与三个 CLI 候选包的不可变制品闭环 |
...
同一批次的全部实现提交共享该批目标版本。批次完成时同步根 `VERSION`、两个 Frontend package/lockfile、生命周期 CLI、release manifest、镜像和插件元数据；本次规划不修改这些版本文件。
...
2. 共享生命周期涉及宿主进程、路径、Docker CLI/Compose、配置 Secret、操作锁与资源清理；它是全部产品操作的控制面，不能分散进各平台脚本。
...
shared lifecycle controller
  ├─ host/daemon/arch/path/port/disk preflight
  ├─ private config + operation lock + ownership state
  ├─ pull images by immutable digest
  └─ docker compose --project-name <owned-project>
          │
          ▼
...
- 交付包中的 release manifest 是版本、源码 revision、Compose 资产、各平台 CLI checksum、各逻辑镜像 index/platform digest、第三方镜像 digest、插件 catalog 和受支持升级来源的唯一发布事实。
- 产品运行只接受 manifest 中的 immutable digest，不以可变 `latest`、本地同名 tag、可写插件卷或上传包建立信任；候选到正式发布只移动引用，不重新构建内容。
- 生命周期 CLI 只调用 Docker Engine 与 Compose，不挂载 Docker socket进任何产品容器，不让 Frontend、Backend 或 Monitor取得宿主编排权限。
...
- `lifecycle/` 使用一个 Go module 和一个 `cmd/gopulse` 命令树，构建 `linux/amd64`、`darwin/arm64`、`windows/amd64` 三个可执行文件。
- 三个交付包除可执行文件名/平台 metadata 外使用相同 Compose、manifest 和文档；路径分隔、process group、console signal 和权限差异封装在最小 platform adapter 中。
- 产品交付包不包含项目源码、Node modules、Go cache、开发 `.env.example` 固定 Secret、测试凭据、source map、宿主绝对路径或 registry credentials。
- 包 checksum、CLI `version --json` 和 release manifest 的 version/revision 必须一致；错误平台可执行文件在访问 Docker 前拒绝。
...
| 命令 | 产品语义 |
| --- | --- |
| `version` | 输出 CLI、manifest、Compose schema 与目标平台版本；不访问 Docker即可运行 |
...
- 配置文件在同目录临时写入、同步并原子替换；POSIX 使用最小权限，Windows 以当前用户 ACL 和加密 backup补足跨平台权限差异。若无法建立安全父目录，初始化失败。
...
- `init` 不自动创建超级管理员。系统启动后按 Phase 15 合同注册首用户并通过受控 CLI operation声明 bootstrap；生命周期只提供受限封装，不在配置中写账号密码或直接 SQL 提权。
...
- `SIGINT`/`SIGTERM`/Windows console interrupt 进入有界取消：未提交临时文件清理，已启动的 Compose project按当前命令语义保留诊断或回到稳定状态，绝不扩大到其他 project。
- `up` 失败保留当前项目资源供 `status/logs/doctor` 诊断；只有用户显式 `down` 才停止。`backup/restore/upgrade` 各自使用第 10～11 节的更严格失败规则。
- 现有 Bash 脚本可继续服务源码开发与历史验收，但产品文档只介绍 `gopulse` CLI；不得让 CLI反向调用 `scripts/*.sh` 或 `scripts/*.ps1` 承担业务逻辑。
...
每个宿主都必须从同一候选 release manifest对应的原生 host bundle开始，并至少执行：
...
- 持久与生命周期：保留卷 `down → up`、单服务替换、SIGINT/SIGTERM/Windows console interrupt、有界失败诊断与恢复。
...
- macOS：APFS含空格路径、Docker Desktop文件共享、`darwin/arm64` CLI与 `linux/arm64`容器、case-insensitive默认卷路径冲突、Terminal interrupt、窄屏/桌面浏览器。
- Windows：NTFS含空格路径、CRLF checkout/解压后 Compose与配置可读、PowerShell参数/引号、`.exe` console interrupt、Docker Desktop Linux container mode、文件共享、端口占用、ACL、cleanup和两个 Frontend。
...
- 交叉编译、buildx成功、静态 Compose config、QEMU运行、WSL中的 Windows CLI或复制另一个平台 evidence均不能填补缺失项。
...
- Frontend bundle、runtime image、release asset、CLI `--help/version`、API错误、日志、evidence和upgrade/restore报告都必须通过 Secret与私有路径哨兵扫描。
...
- 建立双架构产品镜像、六插件 catalog、三个 host CLI候选包和 immutable release manifest。
- 确认第三方镜像双架构与 registry/digest晋升能力，运行各架构最小容器 smoke。
- 不实现完整生命周期、UI重构、backup或upgrade。

### Phase-16-02：共享产品生命周期与安全初始化闭环

- 实现共享 CLI的 version/doctor/init/up/down/status/logs/verify，唯一 edge与强归属。
...
3. 三个host bundle checksum、CLI version、Compose与manifest一致，交付包不含源码、工具链、固定产品Secret、私有路径或registry凭据。
4. 产品通过digest pull运行，不依赖本地同名tag或源码build；候选晋升到正式版本不重建内容。

### 15.2 生命周期、入口与双 Frontend

1. 三平台均能完成 `doctor/init/up/verify/status/logs/down/up`，无宿主Go/Node/Python/基础设施客户端；正常与失败中断只影响当前project。
...
5. Windows路径/CRLF/ACL/文件共享/PowerShell、macOS arm64/APFS/Terminal、Linux权限/signal/单daemon特有项均有真实结果。
...
| --- | --- | --- |
| Phase-16-01 | registry不可变digest/晋升能力、六个第三方镜像双架构支持、buildx platform metadata、三个host CLI构建 | 各逻辑镜像index/platform digest与最小真实run；缺架构则选择最小兼容锁定版本并记录直接回归 |
| Phase-16-02 | 三host Docker/Compose共同子集、Windows process/signal和路径API、macOS/Windows文件共享、Compose pull-by-digest行为 | fake engine self-test + Linux/macOS最小真实clean install；共同子集不足则先修订CLI/支持版本，不写平台分支脚本绕过 |
...
| Phase-16-04 | 锁定MySQL/ES/VM的逻辑dump/snapshot与restore API、队列/offset排空判据、Monitor逻辑export边界、跨平台加密临时文件行为 | 小数据真实backup/restore和故障注入；任一权威历史无法恢复则阻断，不降级为卷tar或“可重建”假设 |
...
- 将根`VERSION`、两个Frontend package/lockfile、lifecycle CLI、manifest、镜像和插件metadata更新为权威表目标版本。
...
- 一个共享产品生命周期、三个host bundle、唯一edge与稳定操作/归属/诊断合同。
