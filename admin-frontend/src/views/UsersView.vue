...
import { ref } from 'vue'
import { requestData, ApiError } from '../services/http'
import { useAuth, leaveManagement } from '../composables/useAuth'
type User={id:number;username:string;role:'user'|'super_admin';created_at:string;is_bootstrap_super_admin:boolean}
const id=ref(''),user=ref<User|null>(null),busy=ref(false),message=ref(''),auth=useAuth()
async function lookup(){if(busy.value)return;user.value=null;message.value='';if(!/^[1-9][0-9]*$/.test(id.value)){message.value='请输入精确正整数用户 ID';return}busy.value=true;try{user.value=await requestData<User>('/admin/users/'+id.value)}catch{message.value='用户不存在或查询失败'}finally{busy.value=false}}
async function change(){const target=user.value;if(!target||busy.value)return;const role=target.role==='user'?'super_admin':'user';if(!window.confirm(`确认将用户 ${target.id} 的角色变更为 ${role}？`))return;busy.value=true;message.value='';try{const result=await requestData<{user:User;changed:boolean}>('/admin/users/'+target.id+'/role',{method:'PUT',body:JSON.stringify({role})});if(target.id===auth.user.value?.id&&role==='user'){user.value=null;await leaveManagement();return}user.value=result.user;message.value=result.changed?'角色已变更':'角色未改变'}catch(e){message.value=e instanceof ApiError&&e.status===409?'引导超级管理员不可降级':'角色变更失败'}finally{busy.value=false}}
</script>
