...
func (f *readOnlyFixture) Request(_ context.Context, req kmsg.Request) (kmsg.Response, error) {
	...
	case *kmsg.MetadataRequest:
		if r.AllowAutoTopicCreation || len(r.Topics) != 1 || r.Topics[0].Topic == nil || *r.Topics[0].Topic != Topic {
			f.t.Fatal("unsafe metadata request")
		}
		topic := Topic
		return &kmsg.MetadataResponse{ControllerID: 1, Brokers: []kmsg.MetadataResponseBroker{{NodeID: 1}}, Topics: []kmsg.MetadataResponseTopic{{Topic: &topic, Partitions: []kmsg.MetadataResponseTopicPartition{{Partition: 0, Leader: 1, Replicas: []int32{1}, ISR: []int32{1}}}}}}, nil
