...
import (
	...
	"github.com/Ray-ymq/GoPulse/exporters/elasticsearch/internal/collector"
)
...
func Load() (Config, error) {
	...
	var err error
	c.ConnectTimeout, err = time.ParseDuration(os.Getenv("ELASTICSEARCH_EXPORTER_CONNECT_TIMEOUT"))
	if err != nil {
		return Config{}, ErrConfig
	}
	c.ScrapeTimeout, err = time.ParseDuration(os.Getenv("ELASTICSEARCH_EXPORTER_SCRAPE_TIMEOUT"))
	if err != nil || c.ConnectTimeout < 100*time.Millisecond || c.ConnectTimeout > c.ScrapeTimeout || c.ScrapeTimeout > 10*time.Second {
		return Config{}, ErrConfig
	}
	host := os.Getenv("ELASTICSEARCH_EXPORTER_HTTP_HOST")
	if host == "" {
		host = "127.0.0.1"
	}
	port := os.Getenv("ELASTICSEARCH_EXPORTER_HTTP_PORT")
	if port == "" {
		port = "9125"
	}
	if (host != "127.0.0.1" && host != "::1") || port != "9125" {
		return Config{}, ErrConfig
	}
...
func Open(c Config) (*collector.Client, error) {
	...
	return &collector.Client{HTTP: client, Origin: "http://" + net.JoinHostPort(c.Host, "9200"), Username: c.Username, Password: c.Password}, nil
}
...
func Handler(db *collector.Client, timeout time.Duration) http.Handler {
	...
	mux.HandleFunc("GET /health", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"status":"ok","service":"elasticsearch-exporter"}`))
	})
