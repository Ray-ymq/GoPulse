...
import (
	...
	runtime "github.com/Ray-ymq/GoPulse/exporters/elasticsearch/internal/runtime"
)
...
func main() {
	if run() != nil {
		if len(os.Args) > 1 {
			...
			componentmetrics.NewLogger("elasticsearch-exporter", os.Stdout).Error("exporter stopped", "reason", "invalid_configuration_or_runtime")
		}
		os.Exit(1)
	}
}
func run() error {
	...
	logger := componentmetrics.NewLogger("elasticsearch-exporter", os.Stdout)
