...
func main() {
	if run() != nil {
		if len(os.Args) > 1 {
			...
			componentmetrics.NewLogger("rabbitmq-exporter", os.Stdout).Error("exporter stopped", "reason", "invalid_configuration_or_runtime")
		}
		os.Exit(1)
	}
}
func run() error {
	...
	logger := componentmetrics.NewLogger("rabbitmq-exporter", os.Stdout)
