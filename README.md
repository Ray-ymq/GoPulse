...
The Backend writes request and business observability records as single-line JSON to stdout. Every record carries `log_schema_version=1`, a UTC RFC3339Nano `timestamp`, lowercase `level`, `service=backend`, a bounded `module`, and a fixed `message`. Call sites cannot replace those reserved fields. Phase 4 intentionally keeps the existing Backend lifecycle and Outbox loop text records until the background-process migration batch; HTTP, business-action, and request-local cache-degradation records already use the JSON schema.
...
The focused isolated acceptance mode validates the correlation and leakage boundaries without touching daily development resources:

```bash
scripts/verify-business.sh --logging-live
```

It exercises registration, login/logout, current-user, post, comment, like, search, notification, representative 400/401/404/503 responses, and a Redis fallback. Panic recovery and request-ID entropy failure are covered by the Backend middleware tests rather than a production debug route.
