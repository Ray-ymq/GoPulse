...
- Docker/Compose-only daily full-stack lifecycle scripts, read-only container verification, deterministic managed-Exporter bootstrap, and random-project real-browser business and observability acceptance matrices;
...
For the focused Phase-12-01 business regression or the authoritative Phase-12-02 observability closure, run:
...
The observability mode builds a fresh random complete stack and drives Chromium through ordinary-user isolation plus administrator Metrics, Logs, Events, and Exporter management. It verifies image users/labels/entrypoints, internal ports and networks, deterministic package/binary identity, managed-plugin bootstrap, VictoriaMetrics/Monitor/Router failure isolation, representative service replacement, Kafka/VM/Elasticsearch/plugin-volume persistence, standalone Exporter target/authentication/recovery/SIGTERM behavior, and a final blank-volume browser install/start/stop/update flow. Destructive operations are constrained by the random Compose project and working-directory labels, and cleanup never prunes shared Docker resources.
...
Phase-12-02 makes `scripts/verify-compose.sh --observability` the authoritative browser-backed container gate. It retains the Phase 11 authorization and trust-boundary checks while replacing the former host-process lifecycle with real non-root images, internal service DNS, managed-plugin volume recovery, and strongly owned Compose cleanup. `scripts/verify-observability-ui.sh` remains a focused historical source-level tool rather than the complete container-runtime proof.
