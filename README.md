...
Backend Schema v1 logs remain single-line JSON on stdout and, when `LOG_MONITOR_URL` is configured, are also offered to a bounded non-blocking in-memory shipper. The shipper uses the dedicated `LOG_MONITOR_INGEST_TOKEN`; queue or transport failures drop or retry only the remote copy and never change API or Outbox results. LogMonitor validates one `service=backend` entry at `POST /internal/v1/logs`, Router transports `logs/backend` envelopes through `gopulse-observability-v1`, and Marshaller stores strict documents in `gopulse-logs-v1-YYYY.MM.DD` behind `gopulse-logs-v1-read`.
