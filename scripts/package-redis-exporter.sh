...
if [[ -n $BINARY ]]; then
  ...
  (cd "$REPO_ROOT/exporters/redis" && CGO_ENABLED=0 GOOS=linux GOARCH="$ARCH" go build -trimpath -buildvcs=false -ldflags='-buildid=' -o "$TEMP_DIR/package/bin/gopulse-redis-exporter" ./cmd/redis-exporter)
fi
...
PY
FILES=(plugin.json bin/gopulse-redis-exporter)
else
  (cd "$REPO_ROOT/monitor" && go run ./cmd/plugin-package-metadata --directory "$TEMP_DIR/package" --version "$VERSION" --arch "$ARCH")
  chmod 0644 "$TEMP_DIR/package/config.schema.json"
  FILES=(plugin.json config.schema.json bin/gopulse-redis-exporter)
fi
